...
class Car:
    ...
    def action(self, choice):
        if choice == 0:
            ...
            self.accelerate(self.dvel)
            self.turn(1) # Rẻ phải theo chiều của xe
...
class RacingEnv:
    ...
    def __init__(self):
        ...
        self.font_smmall = pygame.font.SysFont(None, 25)
    ...
    def render(self, action, start_time):
        ...
        text_surface = self.font_smmall.render(f"Total run time: {time.strftime('%H:%M:%S', time.gmtime(elapsed_time))}", True, pygame.Color('green'))
        self.screen.blit(text_surface, dest=(600, 0))
